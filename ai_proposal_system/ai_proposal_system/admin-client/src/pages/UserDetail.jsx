...
export default function UserDetail() {
  ...
  const fetchAll = useCallback(async () => {
    try {
      const [uRes, vRes, qRes] = await Promise.all([
        getUser(id), getVideos(id), getQuestions(id),
      ]);
  ...
  const handleUploadVideo = async (e) => {
    ...
    setUploadProgress(0);
    try {
      await uploadVideo(id, file, setUploadProgress);
      fetchAll();
    } catch (err) {
    ...
    <Layout>
      ...
      <section style={styles.section}>
        ...
        <div style={styles.uploadArea}>
          <label style={styles.uploadLabel}>
            ...
            <input type="file" accept=".mp4,.mov,.webm" onChange={handleUploadVideo}
      ...
      <section style={styles.section}>
        <div style={styles.sectionHeader}>
          ...
          {!addingQ && (
            <button onClick={() => setAddingQ(true)} style={styles.btnAdd}>+ 질문 추가</button>
          )}
        </div>
                ...
                {editingQ?.id === q.id ? (
                  ...
                  <div style={styles.questionView}>
                    <div style={styles.questionMeta}>
                      ...
                      <span style={q.is_active ? styles.qActive : styles.qInactive}>
                        {q.is_active ? '활성' : '비활성'}
                      </span>
                    </div>
...
const styles = {
  ...
  uploadLabel: {
    display: 'inline-block', padding: '10px 18px', background: '#f0f4ff',
    border: '2px dashed #3498db', borderRadius: 7, cursor: 'pointer', fontSize: 14, color: '#3498db',
  },
  ...
  questionActions: { display: 'flex', gap: 6 },
