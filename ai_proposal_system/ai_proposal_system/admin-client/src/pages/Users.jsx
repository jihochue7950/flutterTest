  ...
  return (
    <Layout>
      <div style={styles.header}>
        ...
        <button onClick={() => navigate('/users/new')} style={styles.btnAdd}>+ 사용자 등록</button>
      </div>
            ...
            <tbody>
              {users.map((u) => (
                <tr key={u.id} style={styles.tr}>
                  ...
                  <td style={styles.td}>
                    <Link to={`/users/${u.id}`} style={styles.link}>{u.name}</Link>
                  </td>
                  <td style={styles.td}>{u.phone || '-'}</td>
                  <td style={styles.td}>{u.email || '-'}</td>
                  <td style={styles.td}>{new Date(u.created_at).toLocaleDateString('ko-KR')}</td>
                  <td style={styles.td}>
                    <Link to={`/users/${u.id}`} style={styles.btnEdit}>상세</Link>
...
const styles = {
  ...
  btnEdit: {
    padding: '4px 10px', background: '#3498db', color: '#fff',
    border: 'none', borderRadius: 4, cursor: 'pointer', fontSize: 12,
    textDecoration: 'none', marginRight: 6, display: 'inline-block',
  },
  btnDel: {
